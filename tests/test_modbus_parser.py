...
def test_modbusparser_parse_value_int16_high():
    ...
    values = ModbusParser.value_to_values(ValueLimit.UINT16, point)
    assert values is not None
    value = ModbusParser.values_to_value(values, point)
    assert value is not None
    assert isinstance(value, int)
    assert value == ValueLimit.UINT16
...
def test_modbusparser_parse_value_int32_high():
    point = PointFactory.create_setpoint(read_length=2)
    values = ModbusParser.value_to_values(ValueLimit.UINT32, point)
    assert values is not None
    value = ModbusParser.values_to_value(values, point)
    assert value is not None
    assert isinstance(value, int)
    assert value == ValueLimit.UINT32
...
def test_modbusparser_parse_value_int64_high():
    ...
    values = ModbusParser.value_to_values(ValueLimit.UINT64, point)
    assert values is not None
    value = ModbusParser.values_to_value(values, point)
    assert value is not None
    assert isinstance(value, int)
    assert value == ValueLimit.UINT64
...
def test_modbusparser_parse_value_float32_high():
    ...
    modbus_value = ValueLimit.UINT32/point.divider
...
def test_modbusparser_parse_value_invalid():
    point = PointFactory.create_setpoint(read_length=2, max=ValueLimit.UINT32_MAXERR)
    modbus_value = ValueLimit.UINT32
