...
def _room(n: int) -> list[Point]:
    base = 100 + (n - 1) * 20
    return [
        Point(f"room_{n}_temp", read=InputRegister(base), data_type=DataType.INT16, scale=0.01, unit=Unit.CELSIUS),
        Point(f"room_{n}_setpoint", read=HoldingRegister(base + 1), write=HoldingRegister(base + 1), data_type=DataType.INT16,
...
def test_a_section_accepts_any_sequence_and_stores_a_tuple() -> None:
    points = [Point("a", read=HoldingRegister(1), data_type=DataType.UINT16)]
...
def test_model_accepts_sequences_and_stores_tuples() -> None:
    identity_point = Point("id_point", read=HoldingRegister(1), data_type=DataType.UINT16)
    section = Section([Point("b", read=HoldingRegister(2), data_type=DataType.UINT16)])
...
def test_a_feature_section_is_included_only_when_its_condition_holds() -> None:
    base = Point("base", read=HoldingRegister(1), data_type=DataType.UINT16)
    cooling = Point("cooling_enabled", read=Coil(1), data_type=DataType.BOOL)
...
def test_a_when_that_raises_is_a_problem_naming_the_section_and_excludes_it() -> None:
    ...
    p = Point("p", read=HoldingRegister(1), data_type=DataType.UINT16)
...
CTS400 = Model(name="CTS400", manufacturer="Nilan",
                sections=[Section([Point("base", read=HoldingRegister(1), data_type=DataType.UINT16)])], options=ModbusOptions(numbering=plain(first_address=1)), read_back_after=1.0)
...
def test_a_factory_that_raises_is_reported_with_its_instance_number() -> None:
    def flaky(n: int) -> list[Point]:
        if n == 2:
            raise ValueError("unlucky")
        return [Point(f"p_{n}", read=InputRegister(n), data_type=DataType.UINT16)]
...
def test_instance_numbers_must_be_unique() -> None:
    model = Model(name="X", manufacturer="Y",
                  sections=[Instances(lambda n: [Point(f"p_{n}", read=InputRegister(n), data_type=DataType.UINT16)],
...
def test_duplicate_keys_across_instances_are_reported() -> None:
    model = Model(name="X", manufacturer="Y",
                  sections=[Instances(lambda n: [Point("shared_key", read=InputRegister(n), data_type=DataType.UINT16)],
...
def test_a_point_already_carrying_the_instance_label_with_a_different_value_is_a_problem() -> None:
    def conflicting(n: int) -> list[Point]:
        return [Point(f"p_{n}", read=InputRegister(n), data_type=DataType.UINT16, labels={"unit": 999})]
...
def test_duplicate_keys_between_identity_and_a_section_are_reported() -> None:
    a = Point("dup", read=HoldingRegister(1), data_type=DataType.UINT16)
    b = Point("dup", read=HoldingRegister(2), data_type=DataType.UINT16)
...
def test_identity_point_without_a_read_side_is_a_problem() -> None:
    write_only = Point("cmd", write=HoldingRegister(1), data_type=DataType.UINT16, write_kind=WriteKind.COMMAND)
...
def test_partial_overlap_of_write_sides_is_a_problem() -> None:
    wide = Point("wide", read=HoldingRegister(100), write=HoldingRegister(100), data_type=DataType.INT32)
    narrow = Point("narrow", read=HoldingRegister(101), write=HoldingRegister(101), data_type=DataType.UINT16)
    model = Model(name="X", manufacturer="Y", sections=[Section([wide, narrow])], options=ModbusOptions(numbering=plain(first_address=1)), read_back_after=1.0)
    found = problems(model, {})
...
def test_identical_spans_are_allowed() -> None:
    view_a = Point("view_a", read=HoldingRegister(100), data_type=DataType.UINT16)
    view_b = Point("view_b", read=HoldingRegister(100), data_type=DataType.UINT16)
...
def test_bit_views_of_one_register_are_allowed() -> None:
    bit_a = Point("bit_a", read=HoldingRegister(50), data_type=DataType.bit(0))
    bit_b = Point("bit_b", read=HoldingRegister(50), data_type=DataType.bit(1))
...
def test_different_address_spaces_never_overlap() -> None:
    in_input = Point("in_input", read=InputRegister(100), data_type=DataType.UINT16)
    in_holding = Point("in_holding", read=HoldingRegister(100), data_type=DataType.UINT16)
...
def test_bit_spaces_allow_the_same_address_for_several_points() -> None:
    d1 = Point("d1", read=DiscreteInput(5), data_type=DataType.BOOL)
    d2 = Point("d2", read=DiscreteInput(5), data_type=DataType.BOOL)
...
def test_the_read_back_delay_is_the_write_effects_then_the_points_then_the_devices() -> None:
    device_delay = Point("a", read=HoldingRegister(1), write=HoldingRegister(1))
    own = Point("b", read=HoldingRegister(2), write=HoldingRegister(2), read_back_after=4.0)
    effect = Point("c", read=HoldingRegister(3), write=HoldingRegister(3), read_back_after=4.0,
...
def test_a_device_read_back_delay_must_be_a_finite_number_of_seconds(seconds: float) -> None:
    model = Model(name="X", manufacturer="Y", sections=[Section([Point("a", read=HoldingRegister(1))])],
...
def test_re_reading_until_stable_needs_a_positive_read_back_delay(seconds: float, refused: bool) -> None:
    mode = Point("mode", read=HoldingRegister(1), write=HoldingRegister(1),
                 on_write=Refresh(["fan"], until_stable=10.0))
    model = Model(name="X", manufacturer="Y", sections=[Section([mode, Point("fan", read=InputRegister(2))])],
                  options=ModbusOptions(numbering=plain(first_address=1)), read_back_after=seconds)
...
def test_on_write_naming_an_unknown_key_is_a_problem() -> None:
    p = Point("cause", read=HoldingRegister(1), write=HoldingRegister(1), data_type=DataType.UINT16,
...
def test_on_change_target_must_select_at_least_one_point() -> None:
    p = Point("trigger", read=DiscreteInput(1), data_type=DataType.BOOL, on_change=Refresh(["missing_key"]))
    model = Model(name="X", manufacturer="Y", sections=[Section([p])], options=ModbusOptions(numbering=plain(first_address=1)), read_back_after=1.0)
    found = problems(model, {})
...
def test_protocol_problems_are_prefixed_with_the_key() -> None:
    ...
    bad = Point("bad", read=HoldingRegister(30001), data_type=DataType.UINT16)
...
def test_points_overlap_by_address_even_when_their_numbers_do_not() -> None:
    ...
    wide = Point("wide", read=InputRegister(19999), data_type=DataType.UINT32)   # addresses 9998-9999
    next_ = Point("next", read=InputRegister(20001))                            # address 9999
...
def test_resolve_succeeds_when_there_are_no_problems() -> None:
    p = Point("ok", read=HoldingRegister(1), data_type=DataType.UINT16)
    model = Model(name="X", manufacturer="Y", sections=[Section([p])], options=ModbusOptions(numbering=plain(first_address=1)), read_back_after=1.0)
...
def test_resolved_model_select_by_keys_preserves_the_given_order() -> None:
    a = Point("a", read=HoldingRegister(1), data_type=DataType.UINT16)
    b = Point("b", read=HoldingRegister(2), data_type=DataType.UINT16)
...
def test_resolved_model_select_by_labels_may_be_empty() -> None:
    a = Point("a", read=HoldingRegister(1), data_type=DataType.UINT16, labels={"room": 1})
    model = Model(name="X", manufacturer="Y", sections=[Section([a])], options=ModbusOptions(numbering=plain(first_address=1)), read_back_after=1.0)
    resolved = resolve(model, {})
...
def test_resolved_model_points_are_in_declaration_order_identity_first() -> None:
    id_point = Point("id_point", read=HoldingRegister(1), data_type=DataType.UINT16)
    section_point = Point("section_point", read=HoldingRegister(2), data_type=DataType.UINT16)
